...
const allowedBlocks = [ 'core/paragraph', 'core/heading', 'core/cover', 'core/button', 'core/list','rblock/highlight'];
const deprecatedBlocks = [ 'rblock/alert-blox' ];
...
const withFontSettings = createHigherOrderComponent( ( BlockListBlock ) => {
	return enhance( ( { select, ...props } ) => {
		...
		if ( allowedBlocks.includes( blockName ) && block?.attributes ) {
			...
			if ( fontFamily ) {
				customData = Object.assign( customData, { 'data-rblock-font': 1 } );
			}
...
addFilter(
	...
	'rblock/withFontSettings',
	withFontSettings
);

addFilter(
	'blocks.getSaveContent.extraProps',
	'rblock/applyFontSettings',
	applyFontSettings
);
