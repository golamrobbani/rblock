 ...
 * @since 1.6.0
 */
class RBlock_Font_Loader {


	/**
	 * This plugin's instance.
	 *
	 * @var RBlock_Font_Loader
	 */
	 ...
	 *
	 * @return RBlock_Font_Loader
	 */
	public static function register() {
		if ( null === self::$instance ) {
			self::$instance = new RBlock_Font_Loader();
		}
	...
	public function fonts_loader() {
		...
		if ( $post && isset( $post->ID ) ) {
			...
			$fonts = get_post_meta( $post->ID, '_rblock_fonts_attr', true );
			...
			if ( ! empty( $fonts ) ) {
				...
				if ( ! empty( $gfonts ) ) {
					...
					wp_enqueue_style( 'rblock-block-fonts' );
				}
